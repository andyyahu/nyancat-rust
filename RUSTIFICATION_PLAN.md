...
### 1. TerminalSession RAII

目前 terminal restore 已從直接 exit 拆出來，但還不是 RAII。下一步可引入：
...
讓一般流程透過 `Drop` 自動 restore terminal 狀態。signal handler 仍保留 raw write + `_exit`，因為 signal path 不適合使用 stdout lock 或一般 Rust cleanup。

完成標準：

- 正常 frame limit return 會自動 restore。
- render error / broken pipe path 行為不變。
- signal handler 邊界保持明確。
- smoke test 覆蓋 frame limit 與 telnet frame limit。

### 2. FrameBuffer 抽象

render 目前仍直接操作 `Vec<u8>`，包含 escape sequence、newline、color transition、counter text。可引入：
...
把 `extend_from_slice`、telnet newline、ANSI reset、counter rendering 這些輸出細節收進方法中。

完成標準：

- render loop 仍重用同一個 buffer，不增加每 frame allocation。
- `push_newline`、counter rendering、color transition 有集中 API。
- 現有 render tests 維持，必要時補 FrameBuffer tests。

### 3. Renderer 與 RenderLoop 拆分
...
- `TerminalSession`：負責 terminal setup / restore。
...
1. TerminalSession RAII
2. FrameBuffer 抽象
3. Renderer / RenderLoop 拆分
4. animation frame consistency tests
5. telnet I/O abstraction
6. sys layer typed fd / signal
7. performance benchmark harness
8. app-level error enum / CLI option spec，視後續複雜度決定
